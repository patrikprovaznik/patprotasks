...
class InterfaceManager:
    ...
    def analyze(self) -> Dict:
        ...
        self.logger.info(f'A total of {len(self.parsed_data)} interfaces were obtained and from that:')
        for key in types:
            self.logger.info(f'{types[key]} interfaces of type {key}.')
        self.logger.info(f'{enabled_count} enabled interfaces.')
        self.logger.info(f'{len(missing_ipv4)} interfaces with missing ipv4 address: {missing_ipv4}')
        self.logger.info(f'{len(missing_ipv6)} interfaces with missing ipv6 address: {missing_ipv6}')
        self.logger.info(f'{len(no_ip_addresses)} interfaces with no ip addresses: {no_ip_addresses}')
...
def main():
    ...
    # Logger
    logger = get_logger(log_path=args.log_path, log_name=__name__, log_level=args.log_level)

    # Setting URL
    set_url = f'{protocol(args.ssl)}://{args.host}:{args.port}/get-all-interfaces'
    url_check(set_url)

    interface_manager = InterfaceManager(set_url, logger)
